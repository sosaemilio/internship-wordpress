...
add_action( 'wp_ajax_psp_lite_update_task_fe', 'psp_lite_update_task_fe' );
function psp_lite_update_task_fe() {
	...
	foreach( $reqs as $post_req ) {
		if( !isset($_POST[$post_req]) ) {
			wp_send_json_error( array( 'message' => __( 'Missing data, please try again', 'psp_projects' ) ) );
		}
	}

	$complete		= strval($_POST['complete']);
	$post_id 		= intval($_POST['post_id']);
	$task_index 	= intval($_POST['task_index']);
	$phase_index 	= intval($_POST['phase_index']);
